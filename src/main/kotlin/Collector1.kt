...
data class Q(val states: Int,
		 ...
		 val fRTI: Double?): Comparable<Q> {

	override fun compareTo(other: Q) =
	...
	override fun toString() =
		listOf(edges, states, symbols, splits, inf, time20?.roundToInt(), states20, edges20, percent20?.mul()?.p(1),
		f20?.p(2), time30?.roundToInt(), states30, edges30, percent30?.mul()?.p(1), f30?.p(2), timeRTI?.roundToInt(), statesRTI,
		edgesRTI, percentRTI?.mul()?.p(1), fRTI?.p(2)).joinToString(" & ", postfix = """ \\\hline""") { if (it == null) "--" else "$it" }
}
...
fun main() {
	...
	print(table.sorted().joinToString("\n"))
}
